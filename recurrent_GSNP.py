...
# from keras_preprocessing.legacy import interfaces
# Due to the keras version problem, we manually put the higher version without the legacy package into keras_preprocessing.
from keras_preprocessing.legacy import interfaces
...
class GSNPCell(Recurrent):
    ...
    def __init__(self, units,
                 ...
                 recurrent_dropout=0.,
                 **kwargs):
        super(GSNPCell, self).__init__(**kwargs)
        self.units = units
        self.activation = activations.get(activation)
    ...
    def build(self, input_shape):
        ...
        self.recurrent_kernel_a = self.recurrent_kernel[:, self.units * 2:]

        if self.use_bias:
            self.bias_r = self.bias[:self.units]
            self.bias_c = self.bias[self.units: self.units * 2]
            self.bias_a = self.bias[self.units * 2:]
        else:
            self.bias_r = None
            self.bias_c = None
            self.bias_a = None
    ...
    def preprocess_input(self, inputs, training=None):
        if self.implementation == 0:
            ...
            x_a = _time_distributed_dense(inputs, self.kernel_a, self.bias_a,
                                          self.dropout, input_dim, self.units,
                                          timesteps, training=training)
            return K.concatenate([x_r, x_c, x_a], axis=2)
        else:
            return inputs
    ...
    def step(self, inputs, states):
        u_tm1 = states[0]  # previous memory
        ...
        if self.implementation == 2:
            ...
            matrix_inner = K.dot(u_tm1 * rec_dp_mask[0],
                                 self.recurrent_kernel[:, :2 * self.units])

            x_r = matrix_x[:, :self.units]
            x_c = matrix_x[:, self.units: 2 * self.units]
            x_a = matrix_x[:, 2 * self.units:]
            recurrent_r = matrix_inner[:, :self.units]
            recurrent_c = matrix_inner[:, self.units: 2 * self.units]
            recurrent_a = matrix_inner[:, 2 * self.units:]

            r = self.recurrent_activation(x_r + recurrent_r)
            c = self.recurrent_activation(x_c + recurrent_c)
            a = self.activation(x_a + recurrent_a)
            ...
            if self.implementation == 0:
                ...
                x_r = K.dot(inputs * dp_mask[0], self.kernel_r)
                x_c = K.dot(inputs * dp_mask[1], self.kernel_c)
                x_a = K.dot(inputs * dp_mask[2], self.kernel_a)
                if self.use_bias:
                    x_r = K.bias_add(x_r, self.bias_r)
                    x_c = K.bias_add(x_c, self.bias_c)
                    x_a = K.bias_add(x_a, self.bias_a)
            else:
            ...
            a = self.activation(x_a + K.dot(u_tm1 * rec_dp_mask[2],
    ...
    def get_config(self):
        config = {'units': self.units,
                  ...
                  'dropout': self.dropout,
                  'recurrent_dropout': self.recurrent_dropout}
        base_config = super(GSNPCell, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
