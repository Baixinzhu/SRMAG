...
def attention(inputs, attention_size, time_major=False, return_alphas=False):
    """
    input:输入数据，是RNN、Bi-RNN层的输出向量，在RNN的情况下，形如[batch_size,max_time,cellsize]
    在Bi-RNN的情况下是包含正向和反向RNN输出张量的元组（outputs_fw,outputs_bw)
    return_alphas：是否将注意力系数变量与层的输出一起返回
    ...
    # w 为权重矩阵，b为偏置向量，u为形状为attention_size的向量
    ...
    # 将每个时间步的向量与注意力向量u相乘得到vu
    vu = tf.tensordot(v, u_omega, axes=1, name='vu')  # (B,T) shape
    # 对vu使用softmax函数计算每个时间步的注意力权重向量alphas
    alphas = tf.nn.softmax(vu, name='alphas')         # (B,T) shape

    # Output of (Bi-)RNN is reduced with attention vector; the result has (B,D) shape
    # 将输入张量与alphas相乘并进行求和，得到最终的输出张量
